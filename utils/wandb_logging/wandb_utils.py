...
class WandbLogger():
    ...
    def setup_training(self, opt, data_dict):
        ...
        if opt.resume_from_artifact:
            ...
            if modeldir:
                self.weights = Path(modeldir) / "fp60_best.pt"
    ...
    def download_model_artifact(self, name):
        ...
        assert model_artifact is not None, 'Error: W&B mymodels artifact doesn\'t exist'
        modeldir = model_artifact.download()
        print("Downloaded mymodels to : ", modeldir)
        return modeldir, model_artifact

    def log_model(self, path, opt, epoch):
        datetime_suffix = datetime.today().strftime('%Y-%m-%d-%H-%M-%S')
        model_artifact = wandb.Artifact('run_' + wandb.run.id + '_model', type='mymodels', metadata={
        ...
        model_artifact.add_file(str(path / 'fp60_best.pt'), name='fp60_best.pt')
        wandb.log_artifact(model_artifact)
        print("Saving mymodels artifact on epoch ", epoch + 1)
