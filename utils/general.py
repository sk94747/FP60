...
def check_imshow():
    ...
    try:
        ...
        cv2.imshow('all_test', np.zeros((1, 1, 3)))
        cv2.waitKey(1)
...
def strip_optimizer(f='fp60_best.pt', s=''):  # from utils.general import *; strip_optimizer()
    # Strip optimizer from 'f' to finalize training, optionally save as 's'
    x = torch.load(f, map_location=torch.device('cpu'))
    if x.get('ema'):
        x['mymodels'] = x['ema']  # replace mymodels with ema
    for k in 'optimizer', 'training_results', 'wandb_id', 'ema', 'updates':  # keys
        x[k] = None
    x['epoch'] = -1
    x['mymodels'].half()  # to FP16
    for p in x['mymodels'].parameters():
...
def print_mutation(hyp, results, yaml_file='hyp_evolved.yaml', bucket=''):
    # Print mutation results to evolve.txt (for use with run_yolov5_train.py --evolve)
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                ...
                # cv2.imwrite('all_test%i.jpg' % j, cutout)
